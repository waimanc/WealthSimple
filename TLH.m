%%%%% Function to calculate the net benefit of TLH.  It is assumed that when
%%%%% an asset is disposed of, a perfectly correlated asset is purchased
function [AfterTaxGrowth AfterTaxGrowthNoTLH...
...
for i = 1:horizon      
    ...
    for j = 1:length(TLH_index)
        ...
        HarvestedAmounts(assetIndex) = HarvestedAmounts(assetIndex) + ...
                              taxRate*initialDeposit*initialWeights(assetIndex)*(CumGrowthAtLastHarvest(assetIndex) - cumGrowth(assetIndex,i))*(cumGrowth(assetIndex,end)/cumGrowth(assetIndex,min(i,i+1)));
